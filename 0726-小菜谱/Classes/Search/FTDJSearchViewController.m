...
@interface FTDJSearchViewController ()<FTDJSearchTableViewControllerDelegate, UITextFieldDelegate>
...
@property (nonatomic, weak) FTDJSearchTableViewController *tableVC;

@end

@implementation FTDJSearchViewController
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        self.view.height = FTDJViewHeight;
...
{
    // 创建一个tableView
    FTDJSearchTableViewController *tableVC = [[FTDJSearchTableViewController alloc] init];
...
#pragma mark - FTDJSearchControllerDelegate代理方法,取消搜索框的第一响应者
...
- (void)searchMenu:(NSString *)menu
{
    ...
    // 搜索结果的表格控制器
    FTDJSearchResultController *searchResultController = [[FTDJSearchResultController alloc] init];
    [self.navigationController pushViewController:searchResultController animated:YES];
    searchResultController.title = menu;
    
    
    // 搜索参数模型
    FTDJSearchParam *param = [[FTDJSearchParam alloc] init];
    param.menu = menu;
    param.key = FTDJAppkey;
    param.rn = @20;
    
    // 搜索菜谱工具类, 搜索菜谱
    [FTDJSearchMenuTool searchMenuWithParam:param success:^(FTDJSearchData *data) {
        ...
        if (data) {
            ...
            if (![self.tableVC.historyMenuList containsObject:menu]) {
                ...
                [FTDJHistoryCacheTool saveHistorySearchKeyWord:menu];
            }
            ...
            if (searchResultController) {
                ...
                // 如果没有找到菜谱数据, 就提示错误信息
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"没有找到菜谱" message:@"请重新输入" delegate:searchResultController cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
                [alert show];
            }
        }
        ...
        if (searchResultController) {
            ...
            // 隐藏蒙版
            [MBProgressHUD hideHUD];
            [searchResultController.tableView addSubview:searchResultController.imageView];
       }
