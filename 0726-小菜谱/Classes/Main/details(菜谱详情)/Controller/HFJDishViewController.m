...
- (void)setFoodModel:(CPData *)foodModel
{
    ...
    // 如果数据库中,有本条菜谱数据
    if ([HFJCollectMenuTool searchMenu:self.foodModel]) {
        // 显示删除按钮
...
- (void)collectOrDelete:(UIButton *)collect
{
    ...
    if ([HFJCollectMenuTool searchMenu:self.foodModel]) {
        ...
        // 从数据库里面搜索到菜谱, 删除收藏的菜谱
        BOOL success = [HFJCollectMenuTool deleteMenu:self.foodModel];
        ...
        // 如果没有搜索到菜谱收藏喜爱的菜谱
        BOOL success = [HFJCollectMenuTool saveCollectMenus:self.foodModel];
